    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Urunler>>(
        ...
        builder: (context,snapshot){
          if(snapshot.hasData){
            var urunlers=snapshot.data;
            return ListView.builder(
                itemCount: urunlers!.length,
                itemBuilder:(context,pos){
                  var urun=urunlers[pos];
                  return GestureDetector(
                    onTap: (){
                      Navigator.push(context, MaterialPageRoute(builder: (context)=>DetaySayfa(urun)));
                    },
                    child: Card(
                      child:Padding(
                        ...
                        child: Row(
                          children: [
                            SizedBox(width: 128, height: 128, child: Image.asset("resimler/${urun.resim}")),
                            Expanded(//resim - ürün bilgileri boşluklarını kullanabilmek için
                              child: Column(
                                mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                crossAxisAlignment: CrossAxisAlignment.center,
                                children: [
                                  Text(urun.ad,style:const TextStyle(fontSize: 16)),
                                  Text("${urun.fiyat} ₺", style:const TextStyle(fontSize: 20,fontWeight: FontWeight.bold),),
